...
- [x] **Elementary (local) — the high-ROI, safe version.** Install the `elementary-data` dbt package
      ...
      → **DONE.** `dbt/packages.yml` (`elementary-data/elementary` 0.25.1, pinned; lock in
      `package-lock.yml`) + the `elementary:` block in `dbt_project.yml` (`+schema: elementary`, so its
      30 models land in their own `main_elementary` schema and never collide with our per-layer
      view/table config). The on-run-end hook is **automatic** — every `dbt build` now appends to
      `main_elementary.elementary_test_results`; verified it captured **all 39** of our tests, with the
      **warn/error severity split preserved** (`assert_lifespan_null_rate_stable`,
      `assert_metric_shape_known` = warn; rest = ERROR). **The guard held:** `Found 39 models, 39 data
      tests` — models 9→39 (our 9 + Elementary's 30), **data tests stayed 39** (a dropped test is the
      failure mode) — `PASS=82 WARN=0 ERROR=0`, `audit.py` green. The HTML report is generated by the
      `edr` CLI via `scripts/observability_report.sh` (isolated `.venv-edr`, so it can't perturb the
      pipeline venv); the 5.6 MB artifact is **regenerable, not committed** — same rule as the dbt docs
      graph (§5). First external dbt package in the project — the deliberate observability exception,
      written up in DECISIONS §3.
- [x] **The debrief framing** (true whether or not the cloud half is built): *GitHub already solves
      ...
      → **Demonstrable now.** The local report IS the point-in-time half — one run's health, generated
      from nothing but the API + `dbt build`. It shows a single invocation because the warehouse is
      discarded each run (§5); the day-over-day trend line is exactly what Part B's durable store
      (MotherDuck) turns on, with zero further model change.
- [x] **Cloud stretch — the durable-storage unlock.** Point the cron at **MotherDuck** so gold + the
      Elementary tables persist past the VM. Then Elementary accumulates **day-over-day trends** and a
      hosted Streamlit reads the same store. Zero model change — materialization is a deployment
      decision.
      → **BUILT + PROVEN live, not "converted the prod target" but cleaner (the cron-only env-var
      seam, my call over the brief's literal wording).** The existing `DBT_DUCKDB_PATH` seam already
      routes an `md:` path to MotherDuck, so the whole change is: the **cron** sets
      `DBT_DUCKDB_PATH=md:dogs` (+ `MOTHERDUCK_TOKEN`), while **ci.yml PR builds stay local/ephemeral**
      — so a PR can never clobber last night's served gold, and §5's "CI proves / cron serves" holds.
      `run_pipeline.sh` gained three md:-aware lines (source `.env`; don't absolutise a connection
      string; `CREATE DATABASE IF NOT EXISTS` once, since MotherDuck doesn't auto-create on attach).
      **Verified from the laptop:** `DBT_DUCKDB_PATH=md:dogs ./scripts/run_pipeline.sh` → 628 raw →
      `Found 39 models, 39 data tests` → `PASS=82 WARN=0 ERROR=0` in the cloud; a plain
