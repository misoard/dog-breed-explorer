...
error) before it touches the warehouse. Beyond the partition the pipeline is **stateless by
deliberate reversal**: every run rebuilds silver/gold from the API as the source of truth, so the
empty warehouse CI hands us is a supported start. What I dropped is not the partition but the
*retention of many partitions as durable history* — an earlier design kept one per day, which on a
source this static would mean bolting persistence (S3 / MotherDuck) on purely to hold it, so
statelessness removes that moving part instead. That deferral of change-history is a real tradeoff
taken for the timeline — worth building later — not a claim the partition is unneeded. **With more
time:** durable storage (MotherDuck / S3) plus dbt SCD-2 snapshots that version a row only when a
breed actually changes, and a `raw.ingestion_runs` audit table in place of stdout logs -- also see
the last section "What I'd build next given another week".
...
(trivial ingestion), and is the modern default for local analytics. The bronze → silver → gold
...
warehouse — fine for a POC. **With more time:** MotherDuck for a hosted shared copy — the same dbt
code, one profile change -- the most important change since it unlocks many things, in particular
would enable us to turn to **incremental** to "only reprocess what changed" (see the last section)
...
on the package for the boilerplate). **With more time:** the specified-but-unbuilt
`mart_temperament_pair_lift`.
...
proves the pipeline; the laptop serves it** — Actions ingests, builds gold, tests, goes green and
then *throws its warehouse away* (needs durable storage), because serving was never its job; the
live Streamlit demo reads the local `dogs.duckdb`. This makes the cloud-handoff problem *vanish*
rather than solving it with MotherDuck/S3/artifacts, and it follows directly from the statelessness
of §1 (every run rebuilds from the API, so a run's warehouse has no unique value worth persisting).
...
and nothing is locked to GitHub Actions. **Traded off:** no hosted/public dashboard URL, and every
PR spends a real API call (no fixture) — the upside is that each check is a genuine end-to-end run
from an empty warehouse. **With more time:** publish the `.duckdb` to MotherDuck / a Release asset
and point a hosted Streamlit at it — turning the same pipeline from *proving* to *serving* with no
model changes — plus an alert on a failed scheduled run.
...
unattended, and freshness is visible from the run status (last run green/red + logs) plus the
`loaded_at` column on the curated data. **With more time:** Dagster, if the DAG grew beyond a
handful of steps or needed richer backfills and observability — but not before, and the difference
is the whole point.
...
drawn from 585 would quietly lie. **Traded off:** no hosted URL, and a deliberately un-clever native
app (no bespoke CSS) that reads plain but stays thin and stable. **Cut, recorded not built:** the
